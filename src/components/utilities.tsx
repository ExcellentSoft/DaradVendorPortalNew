// components/UtilitiesForm.tsx
import React, { useState } from 'react';

// Reusable Card Component - remains generic for other uses
const Card: React.FC<{ title: string; subtitle?: string; children: React.ReactNode; disableStyles?: boolean }> = ({
  title,
  subtitle,
  children,
  disableStyles = false, // Default to false (styles enabled)
}) => (
  // Conditionally apply base card styling based on 'disableStyles' prop
  <div className={disableStyles ? "" : "bg-white p-6 rounded-xl shadow-md border border-gray-100"}>
    {/* Title Text - Will be empty if title prop is empty */}
    <h3 className="text-[#121212] font-bold text-2xl leading-none tracking-wide capitalize mb-2">
      {title}
    </h3>
    {/* Subtitle Text - Will not render if subtitle prop is empty/null */}
    {subtitle && (
...
// Checkmark Icon (for sidebar items)
const CheckmarkIcon: React.FC<{ isActive: boolean }> = ({ isActive }) => (
  <svg
    // Updated color for inactive state from text-[#121212]/40 to text-gray-600
...
// Envelope Icon (No longer used directly in form, kept for completeness if needed elsewhere)
...
interface ServiceCategory {
  id: string;
  name: string;
  formComponent: React.ReactNode; // Component to render when this category is selected
}
...
const UtilitiesForm: React.FC = () => {
  ...
  // CORRECTED: Initialize mobileNumber to an empty string
  const [mobileNumber, setMobileNumber] = useState<string>('');
  const [amount, setAmount] = useState<string>('');
  const [operator, setOperator] = useState<string>('Mtn Nigeria'); // State for selected operator
  ...
  // NEW: State for selected data plan and its error
  ...
  const handleFormSubmit = (e: React.FormEvent) => {
    ...
    // Validate for an 11-digit mobile number starting with '0'
    ...
    if (activeTab === 'airtime') {
      // Basic validation for amount for airtime
      if (!amount || parseFloat(amount) <= 0) {
        setAmountError('Please Enter A Valid Amount.');
        hasError = true;
      }
    } else if (activeTab === 'data') {
      // NEW: Validation for data plan for data top-up
      if (!selectedDataPlan) {
  ...
  // Define the form content for "Airtime & Data Top-Up" with custom header
  const AirtimeDataTopUpForm = (
    // This is the main container for the Airtime & Data Top-Up section.
    // It provides the overall white background and rounded corners for the entire area.
    // Added 'items-center' to horizontally center its flex children
    // Added 'gap-y-8' to restore vertical spacing between header and form body
    <div className="bg-white rounded-xl h-full w-full flex flex-col relative overflow-hidden items-center gap-y-8">
      ...
      {/* NEW DIV: This div specifically houses the form body content with the requested properties */}
      {/*
        MODIFIED:
        - Changed 'py-[60px]' to 'py-[76px]' for top and bottom padding (60px + 16px extra).
      */}
      <div className="w-[400px] rounded-3xl border-[0.5px] border-[#121212]/20 flex flex-col gap-6 py-[24px] px-8"> {/* TARGETED CHANGE */}
        {/* The <form> element is now a flex container to center its child */}
        <form onSubmit={handleFormSubmit} className="flex flex-col items-center justify-center h-full w-full">
          {/* This wrapper div now only manages its internal flex layout and gap,
              its size is indirectly controlled by its parent form's h-full w-full and its grandparent's padding */}
          <div className="flex flex-col gap-6 justify-center items-center">
            {/* Tabs - Removed mb-6, its gap is now managed by the new wrapper */}
            ...
            {/* This div wraps the text, input fields, and button. Removed mt-6, flex-grow, flex-shrink. */}
            <div className="w-[336px] flex flex-col gap-y-6">
              {/* Div for "Recharge mobile numbers instantly" text - with updated typography */}
              ...
              <div className="space-y-6">
                <div>
                  ...
                  {/* Added 'group' class and removed focus-within:ring-2 */}
                  <div className={`group flex w-[335px] mx-auto border rounded-lg focus-within:border-[#1E0156]/50 overflow-hidden gap-3
                    ...
                    {/* Updated +234 div color for error state */}
                    ...
                    {/* Adjusted width for input field to fit the remaining space */}
                    <input
                      type="tel"
                      id="mobileNumber"
                      value={mobileNumber}
  ...
  const serviceCategories: ServiceCategory[] = [
    ...
    },
    {
      ...
      formComponent: OtherServiceForm('Electricity Bill Payments'),
    },
    {
      id: 'api_services_integration',
      name: 'API Services Integration',
      formComponent: OtherServiceForm('API Services Integration'),
    },
  ];
  ...
  return (
    <div className="min-h-screen bg-[#F1F0F2] font-sans text-gray-900 pb-16 max-w-[930px] mx-auto flex flex-col gap-8">
      ...
      <div className="bg-white border border-black/10 w-full h-[114px] pt-7 pr-6 pb-8 pl-6 rounded-lg flex flex-col justify-center items-start text-left">
        {/* Inner DIV for Title and Subtitle */}
      ...
      {/* Main Content Area (the white card with sidebar and form) */}
      <div className="w-full pt-0 pr-0 pb-0 pl-0 rounded-lg flex flex-col lg:flex-row gap-[69px] overflow-hidden">
        {/* Left Column - Sidebar Navigation (the sidebar container div) */}
        {/* Adjusted horizontal padding to accommodate 220px wide options */}
        <div className="w-[249px] h-[214px] pt-[24px] px-[14px] pb-[24px] rounded-xl border bg-white border-black/10 flex-shrink-0 flex flex-col justify-center items-center">
          {/* Card component for the sidebar - title and subtitle props set to empty strings */}
          {/* REMOVED disableStyles={true} from Card component here as it's redundant.
              The parent div already has the styling desired for the sidebar. */}
          <Card
            title=""
            subtitle=""
          >
            {/* Adjusted gap for internal items */}
